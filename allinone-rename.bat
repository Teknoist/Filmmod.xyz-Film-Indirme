...
ffmpeg.exe -v quiet -stats -y -protocol_whitelist file,http,https,tcp,tls,crypto -i https://srv7.filmmodu.xyz/%var%/1080p.en.m3u8 -i %var%.ass -map 0:v -map 0:a -map 1:s -metadata:s:a language=eng -metadata:s:s:1 language=tur -c copy "%dos%.mkv"
del /f %var%.ass
set sonuc=Altyazi
goto end
...
:dublaj
ffmpeg.exe -v quiet -stats -y -protocol_whitelist file,http,https,tcp,tls,crypto -i https://srv7.filmmodu.xyz/%var%/1080p.tr.m3u8 -map 0:v -map 0:a -map -metadata:s:a language=tur -c copy "%dos%.mkv"
